...
const int interruptSensorPin1 = 2;  //  here we will attach the interrupt 
const int interruptSensorPin2 = 3; //  here we will attach the interrupt 
...
void setup() {
  ...
  pinMode(interruptSensorPin1, INPUT_PULLUP);
  pinMode(interruptSensorPin2, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(interruptSensorPin1), handleEmergency, FALLING);
  attachInterrupt(digitalPinToInterrupt(interruptSensorPin2), handleEmergency, FALLING);
...
// left empty idk what to put here
}
...
void loop() {
 ...
 if (emergencyFlag) {
    ...
    emergencyFlag = false; // Reset the flag after handling
  } else {
    BeginSearch();
  }
